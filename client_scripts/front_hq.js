...
NetworkEvents.dataReceived('front:hq_data',event=>{
 ...
 for(var key of ['stateName','enemyName','language','sector','zoneCode','breakthroughPhase','breakthroughZone','breakthroughStrength'])frontHqData[key]=String(d.getString(key));
 for(var key of ['canEdit','warPaused','resetArmed','breakthroughAuto'])frontHqData[key]=d.getBoolean(key);
 for(var key of ['control','garrisonStrength','garrisonMax','warPace','breakthroughDepth','breakthroughMaxDepth','breakthroughLosses','breakthroughLossLimit','breakthroughMinutes'])frontHqData[key]=d.getDouble(key);
...
GUIEvents.createUI('front:headquarters',event=>{
 var t=frontTexts[String(frontHqData.language)] || frontTexts.ru,w=320,h=258
 ...
 if(frontHqTab==='state'){
  ...
  var btPhase=String(frontHqData.breakthroughPhase || 'idle')
  event.label('§6'+t.breakthrough+': §f'+(t['bt_'+btPhase] || btPhase)+(frontHqData.breakthroughZone?' §8| §f'+frontHqData.breakthroughZone:''),x+12,y+164)
  event.label('§8'+(btPhase==='preparing'||btPhase==='assault'?(t.depth+' '+String(frontHqData.breakthroughDepth)+'/'+String(frontHqData.breakthroughMaxDepth)+' | '+t.losses+' '+String(frontHqData.breakthroughLosses)+'/'+String(frontHqData.breakthroughLossLimit)+' | '+String(frontHqData.breakthroughMinutes)+'m'):(btPhase==='cooldown'?String(frontHqData.breakthroughMinutes)+'m':t.admin)),x+12,y+178)
  if(btPhase==='preparing'||btPhase==='assault'){
   event.button(t.bt_cancel,x+12,y+190,296,18).onClick(()=>frontAdminSend('breakthrough_cancel'))
  }else{
   event.button(t.bt_limited,x+12,y+190,92,18).onClick(()=>frontAdminSend('breakthrough_limited'))
   event.button(t.bt_normal,x+114,y+190,92,18).onClick(()=>frontAdminSend('breakthrough_normal'))
   event.button(t.bt_major,x+216,y+190,92,18).onClick(()=>frontAdminSend('breakthrough_major'))
  }
  event.button(t.bt_auto+': '+(frontHqData.breakthroughAuto?'ON':'OFF'),x+12,y+212,296,18).onClick(()=>frontAdminSend('breakthrough_auto'))
 ...
 event.button(t.refresh,x+12,y+236,64,16).onClick(()=>frontHqSend('refresh'))
 event.button('Co-op',x+80,y+236,84,16).onClick(()=>hqPacketOnce('front:coop_ui_request',{action:'view'}))
 event.button('RU',x+176,y+236,40,16).onClick(()=>hqPacketOnce('front:hq_ui_request',{action:'language',language:'ru'}))
 event.button('UK',x+222,y+236,40,16).onClick(()=>hqPacketOnce('front:hq_ui_request',{action:'language',language:'uk'}))
 event.button('EN',x+268,y+236,40,16).onClick(()=>hqPacketOnce('front:hq_ui_request',{action:'language',language:'en'}))
})
